...
# Dan Shea
# 2017.05.06
# 2018.01.30 - Modified to extract all fwd and rev reads so we can make figures
...
# Extract reads for fwd and reverse strands
...
# For reads that map to the reverse strand, we want reads that map in the
# FR orientation.
...
#
	rev1=${output}.rev1.bam
	rev2=${output}.rev2.bam
# Second in the pair, mapped to the reverse strand
	samtools view -@ 10 -b $bam -f 144 -o $rev1
# First in the pair, but NOT mapped to the reverse strand
	samtools view -@ 10 -b $bam -f 64 -F 16 -o $rev2

#
# Combine alignments that originate on the reverse strand.
#
	revout=${output}.rev.bam
	samtools merge -f $revout $rev1 $rev2
	samtools index $revout

# Likewise, for reads that map to the forward strand, we want reads that
...
#
	fwd1=${output}.fwd1.bam
	fwd2=${output}.fwd2.bam
# Second in the pair, but NOT mapped to the reverse strand
	samtools view -@ 10 -f 128 -F 16 -b $bam -o $fwd1
# First in the pair, mapped to the reverse strand
	samtools view -@ 10 -f 80 -b $bam -o $fwd2

#
# Combine alignments that originate on the forward strand.
#
	fwdout=${output}.fwd.bam
